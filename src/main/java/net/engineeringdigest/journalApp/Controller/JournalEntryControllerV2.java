    ...
    public ResponseEntity<?> getById(@PathVariable("myID") String myID) {
        try {
            ...
            if (entry != null) {
                ...
                return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Entry not found with ID: " + myID);
            }
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Error fetching entry: " + e.getMessage());
        }
    }
